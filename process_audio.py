...
class ProcessAudio():
    def __init__(self, mfcc_coefficients):
        ...
        self.audio_labels = ['chroma', 'centroid', 'bandwidth', 'zcr', 'energy', 'contrast', 'rolloff', 'pitch']
    ...
    def _process_audio(self, file_path, label):
        ...
        for feature in self.audio_labels:
            data[feature] = None
        # Load audio file
        audio, sr = librosa.load(file_path, sr=None) # sr=None to retain original sampling rate
        ...
        mfccs = librosa.feature.mfcc(y=audio, sr=sr, n_mfcc=self.mfcc_coefficients)  # Choose desired number of coefficients
        ...
        data['chroma'] = [float(np.mean(np.mean(librosa.feature.chroma_stft(y=audio, sr=sr), axis=1)))]  # Consolidates chroma into one number

        # Compute Spectral Centroid
        data['centroid'] = [np.mean(librosa.feature.spectral_centroid(y=audio, sr=sr))]

        # Compute Spectral Bandwidth
        data['bandwidth'] = [np.mean(librosa.feature.spectral_bandwidth(y=audio, sr=sr))]
        ...
        data['pitch'] = [np.mean(librosa.yin(y=audio, fmin=librosa.note_to_hz('C2'), fmax=librosa.note_to_hz('C7')))]  # Fmin and Fmax should cover most human voice frequencies

        data['label'] = label
        return pd.DataFrame.from_dict(data)
    
    def load_and_export_common_voice_data(self, data, audio_file_directory, output_location=None):
    ...
    def get_deepfake_audio_file_data(self, input_directory, condense_size=1000):
        ...
        for item in real_file_list:
            ...
            real_data = real_data._append({'path': file_path, 'label': 'real'}, ignore_index=True)

        for item in fake_file_list:
            file_path = os.path.join('FAKE', item)
            fake_data = fake_data._append({'path': file_path, 'label': 'fake'}, ignore_index=True)
    ...
    def load_and_export_voice_data(self, data, audio_file_directory, output_location=None):
        if output_location is None:
            output_location = audio_file_directory
    ...
    def get_in_the_wild_audio_file_data(self, input_file, condense_size=1000, input_select=0):
        if input_file.endswith('.tsv'):
            file_data = pd.read_csv(input_file, sep='\t')
        else:
            file_data = pd.read_csv(input_file)

        unique_names = file_data['speaker'].value_counts()  # Grabs the unique celebrity names and how many audio files there are per speaker
        ...
        real_data = file_data.loc[(file_data['label'] == 'bona-fide') & (file_data['speaker'] == unique_names.index[input_select])]
        fake_data = file_data.loc[(file_data['label'] == 'spoof') & (file_data['speaker'] == unique_names.index[input_select])]
        # Shuffle Rows
        real_data = real_data.sample(frac = 1)
        fake_data = fake_data.sample(frac = 1)
        ...
        return pd.concat([real_data, fake_data], ignore_index=True, sort=False), speaker_name
    
    def load_and_export_in_the_wild_data(self, data, audio_file_directory, output_location=None):
        if output_location is None:
            output_location = audio_file_directory
